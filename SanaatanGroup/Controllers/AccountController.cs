...
namespace SanaatanGroup.Controllers
{
    ...
    public class AccountController : Controller
    {
                ...
                case SignInStatus.Success:
                    {
                        ...
                        if (user != null)
                        {
                            if (user.EmailConfirmed == true)
                            {
                                ...
                                ApplicationDbContext db = new ApplicationDbContext();
                                var query = (from u in db.Users where u.Id == user.Id select u).SingleOrDefault();
                                query.LastLoginDate = DateTime.Now;
                                query.LastIPAddress = Request.UserHostAddress;
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, FirstName = model.FirstName, LastName = model.LastName, /*BirthDate = model.BirthDate,*/ PhoneNumber = model.Phone,UserImage=model.UserImage , Gender = model.Gender, LastIPAddress = Request.UserHostAddress, CreatedOn = System.DateTime.Now, EmailConfirmed = true, };
                string Fname = "";
                string path = "";
                ...
                if (pb.ContentLength > 0)
                {
                    ...
                    path = Server.MapPath("~/Content/RegisterImages/");
                    pb.SaveAs(System.IO.Path.Combine(path, Fname));
                    model.UserImage = Fname;
                }
